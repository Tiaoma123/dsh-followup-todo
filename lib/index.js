...
/**
 * 后续待办 host 半（0.1.5 重构版）。
 *
 * 与 0.1.0 旧实现的差异只有两处：
 *  1. 路由改为按 0.1.5 官方写法（dsh-host-open-in-app）显式做浏览器鉴权；
 *  2. 其余（keyOf / 数据文件格式 / 工具名 / 路由名与请求响应形状）逐字保持兼容。
 * 数据文件仍是 ~/.dsh/followup-todos-<base64url(cwd)>.json，绝不使用官方 projectKey()。
 */
 ...
 * ⚠️ 必须原样保留：官方 projectKey() 产出的是 `--home-ubuntu-dsh--`，
 * 与既有数据文件 followup-todos-L2hvbWUvdWJ1bnR1L2RzaA.json 完全不同。
 */
function keyOf(ws) {
...
async function saveList(ws, list) {
  ...
  // 只写 .json；同目录的 <同名>.seeded 标记文件保持不动。
...
/**
 * 浏览器鉴权：0.1.5 的自定义 webServer 路由默认不设防，
 * 官方插件（dsh-host-open-in-app）都显式调用 connection.requestRejection。
 * connection 服务可选，缺席时保持旧行为（不拦截）。
 */
